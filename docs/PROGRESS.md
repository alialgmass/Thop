...
| 5 | Favorites + Comparison | ✅ | Module `Favorites`: `favorites` polymorphic table (`unique(user_id, favoritable_type, favoritable_id)` = BR-FAV-01)، morph map `product`/`supplier` (non-enforcing، AuditLog يفضل FQCN)، `POST/GET/DELETE /api/v1/favorites` (`firstOrCreate` idempotent، `?type=` filter، `FavoritePolicy::delete` owner-only → 403). Module `Comparison`: `GET /api/v1/compare?type=&ids=1,2,3,4` بدون جدول (BR-CMP-01: `max:4` → 400 برسالة واضحة)، بيرجّع `items` + `missing_ids`، بيعيد استخدام `ProductDetailResource`/`SupplierCardResource`. `BusinessAccount::scopeActiveAccount` (owner مش suspended) موحّدة عبر Catalog/Search/Comparison. 15 tests، السويت 260 green. مفيش dependency. Comparison + Favorites تحت `auth:sanctum`. |
